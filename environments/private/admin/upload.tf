resource "helm_release" "upload_service" {
  ...
  name       = "${var.upload_service}-cluster"
  ...
  values = [
    yamlencode({
      image = {
        pull       = var.upload_image_pull
        repository = var.upload_image_url
        tag        = var.upload_image_tag
      }
      securityContext = {
      ...
      service = {
        name      = var.upload_service
        namespace = var.namespace
        version   = var.upload_version
      }
    })
  ]
